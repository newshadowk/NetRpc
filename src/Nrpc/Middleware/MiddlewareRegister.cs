...
namespace Nrpc
{
    ...
    internal class MethodInvokeMiddleware : MiddlewareBase
    {
        ...
        {
            var filters = context.Method.GetCustomAttributes(typeof(NrpcFilterAttribute), true);
            foreach (NrpcFilterAttribute f in filters)
                await f.InvokeAsync(context);
            NrpcContext.ThreadHeader.CopyFrom(context.Header);
